...
class TestXp:
    def test_simple(self, output: StringIO):
        xp(None, o=output)

        assert CRE_ANSI.sub("", output.getvalue()) == ("tests/debugging/test_xp.py:18\n    None (NoneType)\n")

    def test_raw(self, output: StringIO):
        xp(None, o=output)
    ...
    def test_no_space(self, output: StringIO):
        xp(None, o=output)

        assert CRE_ANSI.sub("", output.getvalue()) == ("tests/debugging/test_xp.py:67\n    None (NoneType)\n")

    def test_starred_kwargs(self, output: StringIO):
